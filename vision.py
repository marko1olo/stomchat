...
async def describe_image(file_paths, caption: str = None, is_passive: bool = False) -> str:
    ...
    async with _get_vision_semaphore():
        try:
            ...
            if len(image_urls) > 1:
                album_notice = (
                    f" ВНИМАНИЕ: Загружена серия из {len(image_urls)} снимков/фотографий (клинический фотопротокол/серия RG). "
                    f"Проанализируй каждый снимок последовательно (Снимок 1, Снимок 2 и т.д.), "
                    f"сопоставь динамику (до/после, разные проекции, этапы препарирования/обтурации/фиксации) "
                    f"и дай сравнительное экспертное заключение."
                )
            system_prompt = (
