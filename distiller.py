...
# логировать это явно, а не терять факты молча.
#
# Сейчас список ПУСТ, и это результат, а не упущение: 8.1.1 (Детская), 9.1.1
# (Материаловедение) и 10.1.1 (Прочее, оно же FALLBACK_CODE) в CAT_MAP добавлены,
# то есть каждый лист сита теперь доезжает до файла экспорта. Инвариант стал
# строже: test_distill_pipeline.py [6] сверяет этот список с фактической разницей
# LEAF_CODES и CAT_MAP, поэтому новый лист без рубрики уронит набор, а не уедет
# в тишину. Особенно важно для FALLBACK_CODE: в него попадает всё, что модель не
# смогла классифицировать, и раньше эта корзина не экспортировалась вообще.
NON_EXPORTABLE_LEAVES = frozenset()
