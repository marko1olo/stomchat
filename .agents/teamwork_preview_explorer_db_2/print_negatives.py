...
print("ALL NEGATIVE / FRUSTRATED REACTIONS:")
...
for item in all_neg:
    ...
    print(f"\n[ID: {item['msg_id']}] {item['date']} | @{item['sender_username']} ({item['sender_name']})")
    print(f"  Triggers: {item['triggers']}")
    print(f"  Doctor: \"{item['text']}\"")
    if b_msg:
        print(f"  Bot [{b_msg['msg_id']}]: \"{b_msg['text'][:120]}...\"")
