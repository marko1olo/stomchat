...
print("\n[5a] Рефери не подавляется словом «работа»")
# Рефери гасит конфликты, и обсуждение бота из него исключено, чтобы не
# зациклиться. Но исключение шло подстрокой «бот»: замер по архиву — подавлялся
# на 7119 сообщениях, реально про бота были 98, то есть 99% ложно. Конфликты в
# профессиональном чате как раз вокруг работы и возникают.
for phrase in ["угробить работу технику", "моя работа за субботу",
               "заботиться о пациенте", "обработать канал", "суббота приём"]:
    check(f"«{phrase}» рефери не блокирует",
          not assistant._BOT_REFERENCE_RE.search(phrase.lower()),
          "рефери подавлен обычным словом")
for phrase in ["бот, ты не прав", "это бред от бота", "боты такие боты"]:
    check(f"«{phrase}» рефери исключает", bool(assistant._BOT_REFERENCE_RE.search(phrase.lower())))

print("\n[5b] Список токсичной лексики защищён границами слов")
# Проверено отдельно: он и до правок был сделан правильно — регулярка с
# границей слова и контролируемым набором окончаний. Подозрение на «цирк» в
# «цирконии» не подтвердилось, но проверка остаётся как страховка.
import re as _re
_toxic = ["бред", "чушь", "цирк", "клоун", "олень", "баран", "позор", "срач", "тормоз"]
_pattern = _re.compile(
    rf"\b({'|'.join(map(_re.escape, _toxic))})(е|я|ом|а|ы|и|у|ой|ем|ах|ами|ями|ов|ев)?\b")
for phrase in ["коронка из циркония", "циркониевый каркас", "барабанная перепонка",
               "тормозная жидкость", "позорный результат", "оленью кожу"]:
    check(f"«{phrase}» токсичным не считается", not _pattern.search(phrase.lower()),
          "клиническое слово принято за ругань")
for phrase in ["это бред", "полная чушь", "цирк уехал"]:
    check(f"«{phrase}» распознано как токсичное", bool(_pattern.search(phrase.lower())))
