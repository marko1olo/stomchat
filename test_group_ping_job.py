...
async def run():
    # Тихие часы (22:00–09:00) отключены на время прогона ЯВНО.
    #
    # Без этого тест зависел от настенных часов: днём проходил, а после 22:00
    # джоб корректно выходил по тишине, и пять проверок падали. Любой, кто
    # запустил бы набор вечером, увидел бы «регрессию» там, где её нет.
    # Секция про тихие часы ниже подменяет эту функцию сама.
    assistant.is_ping_quiet_hours = lambda now=None: False
