...
# Фильтр рубрики ищет код ПОДСТРОКОЙ: category_code LIKE '%2.1.2%'. На текущем
# наборе кодов ложных совпадений нет (замер на живой вике: 0 лишних статей), но
# ловушка заряжена. Усечённый код вроде "2.1" утащил бы подстрокой все 2.1.x —
# в одной кнопке оказались бы виниры, коронки, мосты и микропротезирование
# разом. Проверяем именно это условие, а не только пересечение множеств.
_prefix_traps = [(a, b, ca, cb)
                 for a, ca_list in _codes.items() for ca in ca_list
                 for b, cb_list in _codes.items() for cb in cb_list
                 if a != b and ca != cb and ca in cb]
check("ни один код рубрики не является частью другого", not _prefix_traps,
      f"подстрочные пары: {_prefix_traps[:3]}")
check("все коды рубрик трёхуровневые",
      all(c.count(".") == 2 for cl in _codes.values() for c in cl),
      f"не трёхуровневые: {[c for cl in _codes.values() for c in cl if c.count('.') != 2]}")
