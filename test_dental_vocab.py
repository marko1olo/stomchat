...
print("\n[11] Деловые ключи: короткие ищутся только с начала слова")
# BUSINESS_KEYWORDS сверялись подстрокой целиком. Замер на живой базе: «руб»
# сидит в «грубо» (15 раз), «грубости», «зарубеж», «вырубаю», «срубить»;
# «цена» — в «сценарии»; «3d» и «ids» — в ссылках и латинских словах. 31
# реплика чистого трёпа попадала в дайджест всего сообщества только из-за них.
for phrase in ("ой что-то грубо вышло", "я уже даже не вырубаю впн",
               "уехать зарубеж и сняться с учёта", "такой сценарий возможен",
               "не обессудь за грубость", "https://instagram.com/p/dujxzz7ii3d/"):
    check(f"«{phrase[:38]}» в дайджест не идёт",
          summarizer._is_useful_text(phrase) is False)

# Ровно те же ключи в начале слова обязаны работать — иначе фильтр потерял бы
# деловое содержание, ради которого он и заведён.
for phrase in ("3d принтер купил", "цена вопроса 5000", "500 рублей за приём"):
    check(f"«{phrase}» проходит", summarizer._is_useful_text(phrase) is True)

# Остальные ключи ищутся где угодно намеренно: внутри слова у них истинные
# попадания.
for phrase in ("фотопротокол веду всегда", "в поликлинике очередь",
               "микромотор сломался", "себестоимость коронки", "оверпрайс конечно"):
    check(f"«{phrase}» проходит", summarizer._is_useful_text(phrase) is True)

check("списки не пересекаются",
      not (summarizer.BUSINESS_PREFIX_ONLY & summarizer._BUSINESS_ANYWHERE))
check("вместе они дают исходный набор",
      (summarizer.BUSINESS_PREFIX_ONLY | summarizer._BUSINESS_ANYWHERE)
      == set(summarizer.BUSINESS_KEYWORDS),
      "ключ потерялся при разделении")
