...
print("\n[14] Дневной промпт не спорит сам с собой о длине")
# Замер по bot.log, 23 РЕАЛЬНЫЕ дневные генерации: длина ответа модели от 1822 до
# 11264, в среднем 8968. В промпте стояли ДВЕ взаимоисключающие цифры — правило
# оформления требовало «жёсткий лимит 4000-5000 символов», а последняя строка
# задания «в пределах 7000-9000». Модель слушается последней и самой конкретной:
# лимит 4000-5000 нарушен в 22 случаях из 23. После разметки статья упиралась в
# обрезку на 9500, и 10 выпусков из 23 (43%) уходили сообществу с обрубленным
# хвостом: «ЮМОР/ЦИТАТЫ», «ЭКСПЕРТ ДНЯ» и бонусные блоки с пометкой «(В конец)».
# Хуже всех доставалось «МИКРО-ЛАЙФХАКАМ ДНЯ» — они в BONUS_ALWAYS, применимы
# всегда и выбираются чаще прочих.
import io as _io14  # noqa: E402

_SUMM_SRC = _io14.open("summarizer.py", encoding="utf-8").read()
...
check("в дневном промпте нет зашитых цифр длины", not daily_numbers,
      f"got {sorted(daily_numbers)} — цифра снова разъедется с порогом обрезки")
check("длина подставляется из константы",
      daily_prompt.count("{DAILY_CHAR_BUDGET}") >= 2,
      "упоминаний длины меньше двух — либо цифра зашита, либо правило потерялось")
check("константа ниже предела обрезки",
      S.DAILY_CHAR_BUDGET < S.WEEKLY_HTML_LIMIT,
      f"{S.DAILY_CHAR_BUDGET} против {S.WEEKLY_HTML_LIMIT}")
check("запас под разметку не меньше тысячи символов",
      S.WEEKLY_HTML_LIMIT - S.DAILY_CHAR_BUDGET >= 1000,
      f"запас {S.WEEKLY_HTML_LIMIT - S.DAILY_CHAR_BUDGET}, а разметка добавляет ~2%")
check("прежнего лимита 4000-5000 в промпте нет",
      "4000-5000" not in daily_prompt and "4000–5000" not in daily_prompt)
check("прежнего диапазона 7000-9000 в промпте нет",
      "7000-9000" not in daily_prompt and "7000–9000" not in daily_prompt)

# Политика по кейсам была двойной и взаимоисключающей.
check("нет требования схлопнуть кейсы до трёх-четырёх",
      "схлопни их до 3-4" not in daily_prompt)
check("нет требования описать все десять",
      "опиши все 10" not in daily_prompt)
check("политика по кейсам названа однажды и с числом",
      "не больше четырёх" in daily_prompt,
      "без явного числа модель снова возьмёт шесть полей на десять кейсов")
check("объяснено, почему шесть полей на десять кейсов не влезают",
      "18-36 тысяч символов" in daily_prompt,
      "без арифметики требование выглядит произволом и будет снято обратно")

# Противоречие про вступление: запрет фразы и требование той же фразы.
check("запрет и требование одной фразы разведены",
      not ('"Сегодня обсудили"' in daily_prompt and "Не пиши фразы вроде" in daily_prompt),
      "промпт запрещает «Сегодня обсудили» и тут же просит «Сегодня спорили о...»")

print("\n[15] Проверки выше ловят поломку")
check("детектор зашитой цифры сработал бы",
      re.findall(r"(\d{4,5})\s*символ", "не больше 4500 символов") == ["4500"])
check("детектор подстановки различает литерал и константу",
      "{DAILY_CHAR_BUDGET}" not in "не больше 8500 символов")
check("бюджеты обеих веток выведены из одного порога",
      S.WEEKLY_HTML_LIMIT - S.DAILY_CHAR_BUDGET == 1000
      and S.WEEKLY_HTML_LIMIT - S.WEEKLY_CHAR_BUDGET == 1200,
      f"дневной {S.DAILY_CHAR_BUDGET}, недельный {S.WEEKLY_CHAR_BUDGET}, "
      f"порог {S.WEEKLY_HTML_LIMIT}")
