...
"""
        ...
        if (text.lower() == "/sos" or text.lower().startswith("/sos ") or
            ...
            if not sos_arg:
                ...
                is_command = False
                text = (
                    f"Проведи экстренный клинический EBM-разбор ятрогенного осложнения у стоматологического кресла (Chairside Rescue).\n\n"
                    f"Клиническая ситуация:\n{sos_arg}\n\n"
                    f"Структура протокола спасения:\n"
                    f"1. 🚨 <b>Немедленные действия прямо сейчас:</b> что делать врачу в кресле, пока пациент на приёме (промывание, изоляция, временная повязка).\n"
                    f"2. 🛠 <b>Тактика устранения / EBM-протокол:</b> выбор методики (Bypass vs извлечение УЗ, герметизация MTA/Биодентином, гемостаз, медикаментозное прикрытие).\n"
                    f"3. 🗣 <b>Разговор с пациентом (деэскалация):</b> готовая формулировка для врача — как спокойно и профессионально объяснить ситуацию без признания несуществующей вины и без паники.\n"
                    f"4. ⚠️ <b>Красные флаги и чего делать НЕЛЬЗЯ:</b> типичные ошибки, которые ухудшают прогноз (агрессивное давление, перегрев ультразвуком, прогревание, пренебрежение рентгеном).\n"
                    f"5. 📋 <b>Запись в карту и контрольный снимок:</b> как юридически зафиксировать инцидент в Форме 043/у."
                )
        ...
        if (text.lower() == "/translate" or text.lower().startswith("/translate ") or
            ...
            if not trans_arg:
                ...
                is_command = False
                text = (
                    f"Выступи в роли мудрого стоматолога-переводчика и старшего коллеги.\n\n"
                    f"Цитата или жалоба пациента:\n«{trans_arg}»\n\n"
                    f"Структура ответа:\n"
                    f"1. 🧐 <b>Клинический перевод:</b> что это на самом деле означает с точки зрения анатомии, патофизиологии и МКБ-10.\n"
                    f"2. 💬 <b>Как объяснить пациенту:</b> понятный, доброжелательный и профессиональный скрипт для врача у кресла, который развеет заблуждение и вызовет доверие.\n"
                    f"3. 😄 <b>Врачебная ирония / Заметка ординатора:</b> тонкий, остроумный профессиональный стоматологический комментарий (жиза врачебных будней)."
                )
        ...
        if (text.lower() == "/vs" or text.lower().startswith("/vs ") or
            ...
            if not vs_arg:
                ...
                is_command = False
                text = (
                    f"Проведи глубокий сравнительный EBM-батл стоматологических материалов или методик (Material Match).\n\n"
                    f"Сравниваемые позиции:\n{vs_arg}\n\n"
                    f"Структура сравнительного анализа:\n"
                    f"1. 🔬 <b>Физика и механические свойства:</b> прочность на изгиб (МПа), модуль упругости, толщина препарирования, износостойкость.\n"
                    f"2. 🧪 <b>Адгезивный протокол / подготовка поверхности:</b> кислоты (плавиковая HF vs ортофосфорная H3PO4), пескоструй Al2O3, праймеры (10-MDP, силан), сила сцепления.\n"
                    f"3. 🎯 <b>Показания к выбору:</b> когда побеждает вариант А, а когда строго показан вариант Б.\n"
                    f"4. ⚠️ <b>Маркетинг vs Реальность:</b> подводные камни, ограничения и клинические риски, о которых умалчивают на промо-курсах.\n"
                    f"5. 🏆 <b>Вердикт клинициста:</b> четкое резюме по выбору."
                )
...
async def handle_quiz_callback(bot_client, event):
    ...
    if data_str.startswith("sos:"):
        ...
        sos_back_btns = [
            [Button.inline("🚨 К SOS-протоколам", data="nav:sos")],
            [Button.inline("⬅️ В главное меню", data="nav:main")]
        ]
        if sos_kind == "file":
            file_card = (
            ...
            await edit_callback_message(bot_client, event, file_card, "edit_message:sos_file", buttons=sos_back_btns, parse_mode='html')
            await event.answer()
            return
            ...
            await edit_callback_message(bot_client, event, perf_card, "edit_message:sos_perf", buttons=sos_back_btns, parse_mode='html')
            await event.answer()
            return

        elif sos_kind == "sealer":
            sealer_card = (
            ...
            await edit_callback_message(bot_client, event, sealer_card, "edit_message:sos_sealer", buttons=sos_back_btns, parse_mode='html')
            await event.answer()
            return
            ...
            await edit_callback_message(bot_client, event, bleed_card, "edit_message:sos_bleed", buttons=sos_back_btns, parse_mode='html')
            await event.answer()
            return
    ...
    if data_str.startswith("trans:"):
        ...
        trans_back_btns = [
            [Button.inline("🗣 К переводчику", data="nav:translate")],
            [Button.inline("⬅️ В главное меню", data="nav:main")]
        ]
        if trans_kind == "arsenic":
            arsenic_card = (
            ...
            await edit_callback_message(bot_client, event, arsenic_card, "edit_message:trans_arsenic", buttons=trans_back_btns, parse_mode='html')
            await event.answer()
            return

        elif trans_kind == "laser":
            laser_card = (
            ...
            await edit_callback_message(bot_client, event, laser_card, "edit_message:trans_laser", buttons=trans_back_btns, parse_mode='html')
            await event.answer()
            return

        elif trans_kind == "bone":
            bone_card = (
            ...
            await edit_callback_message(bot_client, event, bone_card, "edit_message:trans_bone", buttons=trans_back_btns, parse_mode='html')
            await event.answer()
            return

        elif trans_kind == "nerve":
            nerve_card = (
            ...
            await edit_callback_message(bot_client, event, nerve_card, "edit_message:trans_nerve", buttons=trans_back_btns, parse_mode='html')
            await event.answer()
            return
    ...
    if data_str.startswith("vs:"):
        ...
        vs_back_btns = [
            [Button.inline("⚖️ К батлам материалов", data="nav:vs")],
            [Button.inline("⬅️ В главное меню", data="nav:main")]
        ]
        if vs_kind == "ceramics":
            ceramics_card = (
            ...
            await edit_callback_message(bot_client, event, ceramics_card, "edit_message:vs_ceramics", buttons=vs_back_btns, parse_mode='html')
            await event.answer()
            return

        elif vs_kind == "adhesion":
            adhesion_card = (
            ...
            await edit_callback_message(bot_client, event, adhesion_card, "edit_message:vs_adhesion", buttons=vs_back_btns, parse_mode='html')
            await event.answer()
            return
            ...
            await edit_callback_message(bot_client, event, sealer_card, "edit_message:vs_sealer", buttons=vs_back_btns, parse_mode='html')
            await event.answer()
            return
            ...
            await edit_callback_message(bot_client, event, mta_card, "edit_message:vs_mta", buttons=vs_back_btns, parse_mode='html')
            await event.answer()
            return
