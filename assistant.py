...
# -*- coding: utf-8 -*-
"""
Клинические базы данных StomChat:
- CLINICAL_SOS_CARDS (Экстренные протоколы при осложнениях у кресла)
- PATIENT_TRANSLATION_CARDS (Переводчик с пациентского на клинический язык)
- MATERIAL_BATTLE_CARDS (Батлы стоматологических материалов и методик)
"""
...
CLINICAL_SOS_CARDS = {
    "file": {
        ...
        "text": (
            "🚨 <b>SOS-Протокол: Поломка эндодонтического инструмента (Файлолом)</b>\n\n"
            ...
            "• <b>Фиксация:</b> Сделайте прицельный рентген / визиограмму для определения длины отломка, кривизны канала и локализации (коронковая / средняя / апикальная треть).\n"
            "• <b>Оценка проходимости:</b> Попытайтесь обойти инструмент (Bypass) тонким ручным C-Pilot или K-файлом #06/#08/#10 с обильной ирригацией 17% EDTA.\n\n"
            "<b>2. Алгоритм извлечения или байпаса:</b>\n"
            "• <i>Коронковая / средняя треть (до изгиба):</i> Создание прямой видимости в операционный микроскоп (DOM), формирование плато Gates-Glidden #2/#3, ультразвуковая кавитация сухим ультразвуковым файлом против часовой стрелки.\n"
            "• <i>Апикальная треть (за апексом или за кривизной):</i> При невозможности извлечения без риска перфорации — сформировать <b>Bypass</b>, провести ирригацию NaOCl 3%, обтурировать канал вместе с фрагментом инструмента (прогноз 85-90% при витальном исходном состоянии).\n\n"
            ...
            "В процессе тщательной очистки тончайший никель-титановый инструмент заблокировался в микроканале. "
            "Это штатная техническая сложность в сложной эндодонтии. Мы зафиксировали его положение, канал продезинфицирован, "
            "и сейчас мы выберем наиболее безопасную тактику сохранения зуба без ослабления корня».</i>"
        )
    },
    "perf": {
        "title": "🕳 Перфорация стенки или дна полости зуба",
        "text": (
            "🚨 <b>SOS-Протокол: Перфорация стенки или дна полости зуба</b>\n\n"
            "<b>1. Первая помощь прямо у кресла:</b>\n"
            "• <b>Гемостаз:</b> Обильная ирригация физиологическим раствором (НЕ использовать NaOCl под высоким давлением в зоне перфорации во избежание химического ожога периодонта!).\n"
            "• При упорном кровотечении: стерильный ватный шарик с хлоргексидином 2% или гемостатическая губка с компрессией 2-3 минуты.\n"
            ...
            "• <i>Фуркационная / поддесневая перфорация:</i> Если дефект кости обширен — матрица из рассасывающегося коллагена (Collacone / Spongostan) под дно, затем слой <b>Biodentine</b> или <b>MTA</b> толщиной 2-3 мм с конденсацией влажным бумажным штифтом.\n"
            "• <i>Контроль:</i> Прицельный рентген-контроль плотности адаптации биокерамики.\n\n"
            "<b>3. Скрипт разговора с пациентом:</b>\n"
            "💬 <i>«Из-за глубокого разрушения тканей и близости межкорневой зоны образовалось микросообщение с окружающими тканями. "
            "Мы немедленно закрыли его специальным биоактивным цементом (искусственным дентином), который стимулирует заживление и герметизирует корень. "
            "Нам потребуется наблюдение и контрольный снимок через 3-6 месяцев».</i>"
        )
    },
    "sealer": {
        "title": "⚠️ Выведение силера / гипохлоритовая авария",
        "text": (
            "🚨 <b>SOS-Протокол: Выведение силера / гипохлорита за апекс</b>\n\n"
            "<b>1. Дифференциальная диагностика катастрофы:</b>\n"
            "• <b>Гипохлоритовая авария (NaOCl accident):</b> Внезапная кинжальная боль при ирригации, взрывной отек щеки/губы, гематома, кровотечение из канала.\n"
            "• <b>Выведение эндогерметика (Overfill):</b> Бессимптомно или умеренная механическая постпломбировочная боль при накусывании; критично при попадании в нижнечелюстной канал (парестезия губы/подбородка).\n\n"
            "<b>2. Протокол неотложной помощи:</b>\n"
            "• <i>При аварии с гипохлоритом:</i> Немедленная анестезия (инфильтрация физраствором/артикаином для разведения). Обильное промывание канала стерильным физраствором. Дексаметазон 4-8 мг в/м. Холод на область щеки (15 мин холод / 15 мин перерыв). Назначение анальгетиков (НПВП: Ибупрофен 600 мг или Кеторол) + антигистаминные + антибиотики (Амоксиклав).\n"
            "• <i>При выведении силера в нижнечелюстной канал:</i> Срочная КЛКТ. При неврологическом дефиците (парестезия n. alveolaris inferior) — экстренная консультация челюстно-лицевого хирурга в первые 24–48 часов для декомпрессии нерва.\n\n"
            "<b>3. Скрипт разговора с пациентом:</b>\n"
            "💬 <i>«В области верхушки корня имеется выраженное воспаление и естественное расширение костного канала. "
            "Небольшая часть лечебного герметика вышла за пределы корня в зону воспаления. Это может давать болезненность при накусывании в течение 3-5 дней. "
            "Мы назначили противовоспалительную терапию для быстрого снятия симптомов».</i>"
        )
    },
    "bleed": {
        "title": "🩸 Кровотечение из лунки удаленного зуба",
        "text": (
            "🚨 <b>SOS-Протокол: Упорное кровотечение из лунки удаленного зуба</b>\n\n"
            "<b>1. Первичная экспресс-оценка:</b>\n"
            "• Измерить артериальное давление (АД)! Если АД > 150/90 мм рт. ст. — купирование гипертонического криза (Каптоприл 25 мг под язык или Моксонидин 0.2 мг).\n"
            "• Уточнить анамнез: прием антикоагулянтов/антиагрегантов (Варфарин, Ксарелто, Эликвис, Аспирин).\n\n"
            "<b>2. Местный хирургический протокол (Step-by-Step):</b>\n"
            "1. Провести кюретаж лунки под местной анестезией (без вазоконстриктора или с 1:200 000), удалить рыхлые неполноценные сгустки.\n"
            "2. Ревизия костных стенок: при кровотечении из костного сосуда — смятие края кости гладилкой/кюретой или внесение костного воска (Bone Wax).\n"
            "3. Внесение гемостатического агента: гемостатическая губка с коллагеном и тромбином (Тахокомб, Спонгостан, Альвостаз) + смочить раствором <b>Транексамовой кислоты</b> (50 мг/мл).\n"
            "4. <b>Глухое ушивание лунки:</b> Наложение тугих крестообразных или 8-образных швов (Vicryl 3-0 или 4-0), сближение краев десны.\n"
            "5. Давящий марлевый тампон с транексамом прикусить плотно на 20–30 минут в кресле под контролем врача.\n\n"
            "<b>3. Домашние назначения:</b>\n"
            "• Полоскания транексамовой кислотой (5% раствор) 3-4 раза в день (не сплевывать активно, удерживать во рту 2 минуты).\n"
            "• Исключить физические нагрузки, горячую ванну, алкоголь на 48 часов."
        )
    },
    "aspiration": {
        "title": "🫁 Аспирация / проглатывание предмета (бор, кламмер)",
        "text": (
            "🚨 <b>SOS-Протокол: Аспирация или проглатывание инструмента в кресле</b>\n\n"
            "<b>1. Немедленные действия прямо сейчас:</b>\n"
            "• <b>Оценка проходимости дыхательных путей:</b> Если пациент кашляет — стимулируйте активный кашель. Не стучите по спине в сидячем положении (риск смещения предмета глубже в бифуркацию трахеи)!\n"
            "• <b>При острой обструкции (не может говорить, синеет):</b> Немедленно прием Геймлиха (толчки в эпигастрий снизу вверх) стоя или в кресле.\n"
            ...
            "• <i>Аспирация:</i> Приступообразный судорожный кашель, одышка, свистящее дыхание, стридор. Экстренный вызов скорой медицинской помощи (112 / 03), срочная госпитализация в торакальное отделение, бронхоскопия.\n"
            "• <i>Проглатывание:</i> Предмет проскользнул в пищевод (кашля нет, затруднено глотание). Обязательное направление на рентгенографию органов брюшной полости и грудной клетки для локализации предмета (ФГДС для извлечения из желудка).\n\n"
            "<b>3. Юридическая фиксация и разговор:</b>\n"
            "💬 <i>«Во время процедуры соскочил мелкий фиксатор. Чтобы исключить любые риски для пищевода, мы сейчас организуем контрольный снимок и осмотр в специализированном отделении. Мы сопровождаем вас на каждом этапе».</i>"
        )
    },
    "anesthesia_failure": {
        "title": "⚡️ «Горячий пульпит» — не берет анестезия",
        "text": (
            "🚨 <b>SOS-Протокол: Неэффективность анестезии при «горячем пульпите»</b>\n\n"
            "<b>1. Патофизиология отказа:</b>\n"
            "При выраженном воспалении pH тканей падает до 5.5–6.0. Молекула артикаина/лидокаина не может диссоциировать в липофильное основание и не проникает через мембрану аксона. Кроме того, экспрессируются ТТХ-резистентные натриевые каналы (Nav 1.8 / Nav 1.9).\n\n"
            "<b>2. Дополнительные ступени обезболивания:</b>\n"
            "1. <b>Мандибулярная блокада по Гоу-Гейтсу (Gow-Gates) или Вазирани-Акинози (Vazirani-Akinosi):</b> Введение анестетика к шейке мыщелкового отростка выше зоны воспаления.\n"
            "2. <b>Интралигаментарная анестезия (PDL):</b> Введение артикаина 4% под высоким давлением (шприц Citoject / обычный карпульный) в 4 точки периодонтальной связки зуба под углом 30° к оси корня.\n"
            "3. <b>Внутрикостная анестезия (Quicksleeper / X-Tip):</b> Перфорация кортикальной пластинки межзубной перегородки и введение депо анестетика прямо в губчатую кость (наступает за 30 секунд).\n"
            "4. <b>Внутрипульпарная анестезия:</b> После точечного вскрытия крыши пульпарной камеры — плотное заклинивание тонкой иглы 30G в устье и введение 0.2 мл под резким гидродинамическим давлением (секундная боль сменяется мгновенной анестезией).\n\n"
            "<b>3. Фармакологический лайфхак:</b> Премедикация Ибупрофен 600–800 мг или Кеторол за 30–45 минут до приёма снижает уровень простагландинов E2 и восстанавливает чувствительность рецепторов."
        )
    },
    "emphysema": {
        "title": "💨 Подкожная эмфизема лица и шеи",
        "text": (
            "🚨 <b>SOS-Протокол: Ятрогенная подкожная эмфизема мягких тканей</b>\n\n"
            "<b>1. Как возникает и симптомы:</b>\n"
            "Попадание сжатого воздуха из турбинного наконечника, пустера или Air-flow в клетчаточные пространства через разрез, перфорацию или периодонтальный карман (особенно в зоне 3.8/4.8).\n"
            "• <b>Симптомы:</b> Быстро нарастающий безболезненный отек щеки, поднижнечелюстной зоны, шеи. Характерный симптом — <b>пальпаторная крепитация (хруст сухого снега)</b> под кожей.\n\n"
            ...
            "3. <b>Категорически запретить пациенту:</b> полоскать рот под давлением, сморкаться, накладывать согревающие компрессы (риск инфицирования и флегмоны)!\n"
            "4. Назначить антибиотикопрофилактику: Амоксиклав 1000 мг 2 раза в день 7 дней (воздух из турбинки нестерилен, риск медиастинита!).\n"
            "5. Ингаляция 100% увлажненным кислородом ускоряет рассасывание азота в тканях в 3 раза.\n\n"
            "<b>3. Маршрутизация:</b> При распространении отека на шею ниже подъязычной кости — экстренная госпитализация в ЧЛХ-стационар."
        )
    },
    "sinus_perf": {
        "title": "🦴 Перфорация гайморовой пазухи (Ороантральное соустье)",
        "text": (
            "🚨 <b>SOS-Протокол: Перфорация дна верхнечелюстной пазухи при удалении</b>\n\n"
            "<b>1. Диагностика прямо у кресла:</b>\n"
            "• <b>Носоротовая проба Вальсальвы:</b> Пациент зажимает нос пальцами и пытается плавно выдохнуть через нос. При перфорации — свист воздуха, пузырение крови в лунке. (Внимание: не дуть слишком сильно!).\n"
            "• <b>Зондирование:</b> Осторожное зондирование лунки тупым хирургическим зондом — ощущение «проваливания в пустоту».\n\n"
            "<b>2. Тактика в зависимости от размера дефекта:</b>\n"
            "• <i>Микроперфорация (< 2 мм):</i> Формирование полноценного кровяного сгустка, гемостатическая губка (Альвостаз / Спонгостан) в устье лунки, крестообразный шов через десневые сосочки без натяжения. Заживает самостоятельно в 90% случаев.\n"
            "• <i>Дефект среднего и крупного размера (3–5+ мм):</i> Немедленная пластика соустья трапециевидным слизисто-надкостничным щечным лоскутом по Вассмунду (с рассечением надкостницы для скольжения) или перемещенным небным лоскутом. Глухой шов Vicryl 4-0.\n\n"
            "<b>3. Обязательный домашний охранительный режим:</b>\n"
            "• <b>Запрет сморкания, чихания с закрытым ртом, надувания щек и питья через трубочку на 14 дней.</b>\n"
    ...
    "torque_loss": {
        ...
        "text": (
            "🚨 <b>SOS-Протокол: Прокручивание имплантата и срыв торка (Spinning Implant)</b>\n\n"
            "<b>1. Причина катастрофы:</b>\n"
            "Мягкий тип кости (D3/D4 на верхней челюсти), перерасширение костного ложа фрезами (over-drilling), избыточная коническая развертка, дефект кортикальной пластинки.\n\n"
            "<b>2. Алгоритм действий хирурга:</b>\n"
            "1. <b>Выкрутить имплантат:</b> Не оставлять прокручивающийся имплантат на удачу — фиброинтеграция гарантирована в 95% случаев.\n"
            "2. <b>Замена на больший диаметр:</b> Установка имплантата на 0.5–1.0 мм шире (например, планировался 3.8 мм → взять 4.3 или 4.5 мм той же длины).\n"
            "3. <b>Замена на большую длину:</b> Если позволяет расстояние до нижнечелюстного канала / синуса — пройти глубже на 2 мм для зацепления за базальную кость.\n"
            "4. <b>Техника недопрепарирования (Undersizing):</b> Новое ложе формируется без финальной профильной фрезы и без метчика.\n"
            "5. <b>Только двухэтапный протокол:</b> Установка винта-заглушки (Cover screw), глухое ушивание десны. Категорически запрещена немедленная нагрузка и установка формирователя десны!"
        )
    },
    "dislocation": {
        "title": "💥 Острый вывих нижней челюсти в кресле",
        "text": (
            "🚨 <b>SOS-Протокол: Передний вывих височно-нижнечелюстного сустава (ВНЧС)</b>\n\n"
            "<b>1. Как распознать:</b>\n"
            "После широкого открывания рта (удаление восьмерки, снятие оттисков) пациент не может закрыть рот. Подбородок выдвинут вперед, обильное слюнотечение, напряжение жевательных мышц, пальпируется западение кпереди от козелка уха.\n\n"
            "<b>2. Вправление методом Гиппократа (Step-by-Step):</b>\n"
            "1. Усадить пациента на низкий стул, голова плотно прижата к подголовнику.\n"
            "2. <b>Защита пальцев врача:</b> Оберните большие пальцы обеих рук толстым слоем марлевых салфеток (при вправлении челюсть рефлекторно захлопывается с силой до 70 кг!).\n"
            "3. Поместите большие пальцы на косые линии нижней челюсти кнаружи от нижних моляров, остальными пальцами обхватите нижний край челюсти снизу.\n"
            "4. <b>Вектор тяги:</b> Плавно и мощно надавите вниз (преодолевая спазм жевательных мышц), затем назад и слегка вверх. Головка мыщелка со щелчком соскальзывает обратно в суставную ямку.\n\n"
...
PATIENT_TRANSLATION_CARDS = {
    "arsenic": {
        "title": "☠️ «Положите мне мышьяк, как раньше»",
        "text": (
            "🗣 <b>Пациент:</b> <i>«Доктор, положите мне мышьяк, как раньше, а пломбу потом!»</i>\n\n"
            "📋 <b>Клинический декодер (МКБ-10):</b>\n"
            "• Диагноз: <b>K04.0 Пульпит</b> (острый очаговый или диффузный пульпит, K04.01 / K04.02).\n"
            "• EBM-реальность: Мышьяковистый ангидрид (As2O3) исключен из современных мировых протоколов из-за риска токсического периодонтита, некроза межзубного сосочка и остеонекроза челюсти. Золотой стандарт — витальная экстирпация пульпы под адекватной проводниковой/инфильтрационной анестезией артикаином 4% за одно посещение.\n\n"
            "💬 <b>Скрипт для врача (эмпатично и убедительно):</b>\n"
            "<i>«Я понимаю ваше желание решить вопрос быстро, но мышьяк — это токсичный яд из прошлого века. "
            "Сегодня мы обезболим зуб мягким современным анестетиком, полностью уберем воспаленный нерв за 20 минут "
            "и запечатаем каналы биологически нейтральным материалом, чтобы зуб служил вам десятилетиями без токсических осложнений».</i>\n\n"
            "😄 <b>Врачебная жиза:</b>\n"
            "<i>Мышьяк в современной стоматологии остался в том же музее, где лежат ножной бор с ременным приводом и стеклянный шприц Рекорд с кипячением в эмалированном лотке.</i>"
        )
    },
    "laser": {
        "title": "⚡️ «Световая пломба лазером без сверления»",
        "text": (
            "🗣 <b>Пациент:</b> <i>«Мне нужна световая пломба лазером, только без бормашины и сверления!»</i>\n\n"
            "📋 <b>Клинический декодер (МКБ-10):</b>\n"
            "• Диагноз: <b>K02.1 Кариес дентина</b> / <b>K02.2 Кариес цемента</b>.\n"
            "• EBM-реальность: Пациент путает фотополимеризационную светодиодную лампу (длина волны 440–480 нм) с препарирующим эрбиевым лазером (Er:YAG 2940 нм). Невозможно поставить композитную реставрацию без тщательного иссечения инфицированного размягченного дентина твердосплавными или полимерными борами.\n\n"
            "💬 <b>Скрипт для врача (на пальцах без занудства):</b>\n"
            "<i>«Свет синей лампы — это не лазер, который режет, а специальный безопасный световой активатор, который за секунды превращает мягкий нанокомпозит в монолитную прочную эмаль. "
            "Но чтобы пломба держалась герметично и под ней не развился скрытый кариес, нам нужно бережно вычистить микробный налет микроборами под водяным охлаждением. Вы ничего не почувствуете благодаря анестезии».</i>\n\n"
            "😄 <b>Врачебная жиза:</b>\n"
            "<i>Лазерная пломба без сверления — любимый миф маркетинга 2000-х. Пациент уверен, что доктор посветит синей указкой — и кариозная полость растворится в параллельной вселенной.</i>"
        )
    },
    "bone": {
        "title": "🦴 «Кость во рту рассосалась»",
        "text": (
            "🗣 <b>Пациент:</b> <i>«Прошлый доктор сказал, что у меня кость во рту рассосалась и зубы скоро выпадут...»</i>\n\n"
            "📋 <b>Клинический декодер (МКБ-10):</b>\n"
            "• Диагноз: <b>K05.3 Хронический пародонтит</b> (генерализованный/локализованный) / <b>K08.2 Атрофия беззубого альвеолярного края</b>.\n"
            "• EBM-реальность: Горизонтальная или вертикальная резорбция альвеолярной кости вызвана хронической иммунно-воспалительной реакцией на поддесневую биопленку (P. gingivalis, T. forsythia). Кость не «рассасывается от старости», а подвергается остеокластической резорбции из-за микробов.\n\n"
            "💬 <b>Скрипт для врача (деэскалация тревоги):</b>\n"
            "<i>«Костная ткань не исчезает сама по себе. Под десной скапливаются бактерии и зубной камень, которые вызывают воспаление. "
            "Организм, защищаясь от микробов, отодвигает уровень кости дальше от инфекции. Мы остановим этот процесс: проведем глубокое ультразвуковое очищение корней (SRP), устраним воспаление десны и стабилизируем связочный аппарат зубов».</i>\n\n"
            "😄 <b>Врачебная жиза:</b>\n"
            "<i>Пациенты представляют «рассасывание кости» как кусок сахара в горячем чае. Наша задача — объяснить, что кость не сахар, а зубы спасаются регулярной гигиеной и кюретажем, а не чудодейственной пастой из телерекламы.</i>"
        )
    },
    "nerve": {
        "title": "❄️ «Нерв в десне простудил на сквозняке»",
        "text": (
            "🗣 <b>Пациент:</b> <i>«У меня зуб не гнилой, я просто нерв в десне простудил на сквозняке в машине»</i>\n\n"
            "📋 <b>Клинический декодер (МКБ-10):</b>\n"
            "• Диагноз: <b>K04.0 Острый пульпит</b> или <b>K04.5 Хронический апикальный периодонтит в стадии обострения</b>.\n"
            "• EBM-реальность: Никакого «простуженного нерва в десне» не существует. Холодный воздух — лишь температурный триггер, который раздражает C- и A-дельта волокна уже необратимо воспаленной пульпы через микротрещину эмали или скрытый контактный кариес.\n\n"
            "💬 <b>Скрипт для врача:</b>\n"
            "<i>«Холодный воздух на сквозняке — это лишь индикатор, как звонок будильника. Сам по себе нерв простудиться не может. "
            "Холод среагировал на скрытый воспалительный процесс глубоко внутри зуба под эмалью или пломбой. "
            "Давайте сделаем снимок и найдем истинную причину, пока зуб не разболелся ночью непрерывно».</i>\n\n"
            "😄 <b>Врачебная жиза:</b>\n"
            "<i>У пациентов на сквозняке простужаются: нервы, челюсти, кости, корни и даже пломбы 10-летней давности. Но снимок всегда предательски показывает глубокую контактную полость со вскрытым рогом пульпы.</i>"
        )
    },
    "calcium": {
        "title": "🍼 «Зубы покрошились, ребенок кальций высосал»",
        "text": (
            "🗣 <b>Пациент:</b> <i>«У меня зубы стали крошиться, потому что во время беременности ребенок весь кальций высосал!»</i>\n\n"
            "📋 <b>Клинический декодер (МКБ-10):</b>\n"
            "• Диагноз: <b>K02.1 Кариес дентина</b>, множественный цветущий кариес / <b>K03.2 Эрозия зубов</b>.\n"
            "• EBM-реальность: Плод физиологически НЕ может забирать кальций из эмали зубов матери — в зрелой эмали нет кровеносных сосудов! Разрушение зубов связано с тремя факторами: токсикоз с частой рвотой (соляная кислота HCl разрушает эмаль), частые углеводные перекусы с падением pH слюны и снижение выработки слюны.\n\n"
            "💬 <b>Скрипт для врача:</b>\n"
            "<i>«Малыш формирует свои кости из кальция вашей крови и питания, а не из зубной эмали. Зубы пострадали из-за изменения кислотности слюны и токсикоза во время беременности. "
            "Мы укрепим эмаль минеральными лаками, восстановим поврежденные зубы и подберем реминерализирующую защиту».</i>\n\n"
            "😄 <b>Врачебная жиза:</b>\n"
            "<i>По версии пациентов, эмбрион ночью тайно бурит эмаль матери алмазным бором. Но в реальности эмаль растворяется чаем с печеньем в 3 часа ночи во время кормления грудью.</i>"
        )
    },
    "milk_teeth": {
        "title": "👶 «Зачем молочный лечить, он же выпадет»",
        "text": (
            "🗣 <b>Пациент:</b> <i>«Зачем мучить ребенка и ставить пломбы на молочные зубы? Они же всё равно выпадут!»</i>\n\n"
            "📋 <b>Клинический декодер (МКБ-10):</b>\n"
            "• Диагноз: <b>K04.5 Хронический апикальный периодонтит временного зуба</b> / <b>K07.3 Аномалии положения зубов</b>.\n"
            "• EBM-реальность: Корни временных моляров охватывают зачаток постоянного зуба. Гнойное воспаление приводит к гибели зачатка или фолликулярной кисте (зуб Турнера). Раннее удаление молочного зуба приводит к мезиальному сдвигу постоянной шестерки и тяжелой скученности зубного ряда.\n\n"
            "💬 <b>Скрипт для врача:</b>\n"
            "<i>«Молочный зуб — это природный хранитель места для постоянного зуба. Если инфекция дойдет до корня, она сожжет зачаток постоянного зуба под ним. "
            "Сохраняя молочный зуб здоровым, мы защищаем постоянный прикус и спасаем ребенка от сложных ортодонтических аппаратов в будущем».</i>\n\n"
            "😄 <b>Врачебная жиза:</b>\n"
            "<i>Молочные зубы действительно выпадут. Но перед этим могут подарить детскому челюстно-лицевому хирургу ночную периостотомию, а ортодонту — путевку на Мальдивы.</i>"
        )
    },
    "adrenalin_allergy": {
        "title": "❤️ «У меня аллергия на весь адреналин»",
        "text": (
            "🗣 <b>Пациент:</b> <i>«Доктор, мне только без адреналина! У меня на него жуткая аллергия, сердце из груди выпрыгивает и руки трясутся!»</i>\n\n"
            "📋 <b>Клинический декодер (МКБ-10):</b>\n"
            "• Диагноз: <b>Z88.8 Ложный аллергологический анамнез</b> / Паническая реакция / Симпатоадреналовый криз.\n"
            "• EBM-реальность: На адреналин не бывает IgE-опосредованной аллергии — это эндогенный нейромедиатор, синтезируемый мозговым веществом надпочечников самого пациента. Сердцебиение вызвано либо выбросом собственного адреналина от страха, либо случайным интравазальным введением при отсутствии аспирационной пробы.\n\n"
            "💬 <b>Скрипт для врача:</b>\n"
            "<i>«Адреналин — это гормон, который ваш собственный организм вырабатывает прямо сейчас от волнения. "
            "То, что вы чувствовали раньше — это естественная реакция сосудов на страх или быстрое введение. Сегодня мы сделаем деликатную анестезию с обязательным аспирационным тестом, медленно и безболезненно».</i>\n\n"
            "😄 <b>Врачебная жиза:</b>\n"
            "<i>Если бы у человека была истинная аллергия на адреналин, он бы не дожил до совершеннолетия, погибнув от собственного страха в первом классе у доски.</i>"
        )
    },
    "vodka_garlic": {
        "title": "🧄 «Полоскал водкой и прикладывал чеснок»",
        "text": (
            "🗣 <b>Пациент:</b> <i>«Зуб разболелся ночью, я прополоскал перцовкой с солью и привязал чеснок к десне, а она побелела и слезла!»</i>\n\n"
            "📋 <b>Клинический декодер (МКБ-10):</b>\n"
            "• Диагноз: <b>T30.0 Химический ожог слизистой оболочки рта</b> + <b>K04.0 Острый пульпит</b>.\n"
            "• EBM-реальность: Этанол концентрацией > 40% и фитонциды чеснока (аллицин) вызывают коагуляционный некроз неороговевающего эпителия слизистой рта за 5–10 минут. Боль в зубе при этом не проходит, но добавляется жестокий ожог надкостницы и десны.\n\n"
            "💬 <b>Скрипт для врача:</b>\n"
            "<i>«Спирт и чеснок сожгли поверхностный слой десны, вызвав химический ожог. Инфекция находится глубоко внутри твердого зуба, куда никакие примочки проникнуть не могут. "
            "Сейчас мы нейтрализуем воспаление в зубе под обезболиванием, а на десну нанесем заживляющий кератопластический гель».</i>\n\n"
            "😄 <b>Врачебная жиза:</b>\n"
            "<i>Пациент пытался вылечить зуб, а приготовил в полости рта маринад для шашлыка с ожогом третьей степени по переходной складке.</i>"
        )
    },
    "cement_forever": {
        "title": "🧱 «Поставьте советский цемент на века»",
        "text": (
            "🗣 <b>Пациент:</b> <i>«Не надо мне ваш световой пластик, он отваливается! Поставьте нормальный советский цемент, он намертво стоит десятилетиями!»</i>\n\n"
            "📋 <b>Клинический декодер (МКБ-10):</b>\n"
            "• Диагноз: <b>K02.1 Кариес дентина</b> / Требование устаревших методик.\n"
            "• EBM-реальность: Советские силикатные цементы (Силидонт) твердели с выделением свободной ортофосфорной кислоты, вызывая некроз пульпы без массивной прокладки, со временем вымывались слюной и не обладали адгезией к тканям зуба. «Стояние на века» объяснялось тем, что пломба держалась механически, пока под ней тихо сгнивал весь корень.\n\n"
            "💬 <b>Скрипт для врача:</b>\n"
            "<i>«Старые цементы держались только за счет глубоких поднутрений и со временем растворялись слюной, создавая скрытый кариес внутри корня. "
            "Современный композит срастается с тканями зуба на микроскопическом уровне благодаря адгезиву, распределяет жевательную нагрузку и защищает зуб от раскола».</i>\n\n"
            "😄 <b>Врачебная жиза:</b>\n"
            "<i>Советский Силидонт действительно стоял 30 лет. Зуб под ним сгнил 25 лет назад, но монолитная пломба гордо висела в воздухе, опираясь на соседний зуб и честное слово.</i>"
        )
    },
    "why_so_expensive": {
        "title": "💰 «А почему так дорого, вы просто посветили?»",
        "text": (
            "🗣 <b>Пациент:</b> <i>«Откуда такая цена за пломбу? Вы же просто посверлили 3 минуты и посветили синей фонариком!»</i>\n\n"
            "📋 <b>Клинический декодер (МКБ-10):</b>\n"
            "• Диагноз: <b>Z76.8 Конфликт ценообразования</b> / Непонимание технологии.\n"
            "• EBM-реальность: Пациент видит только вершину айсберга. В стоимость реставрации заложены: стерилизация по стандартам Анти-СПИД/Анти-Гепатит (автоклавы класса B), латексная изоляция коффердамом, микроскопная оптика, адгезивная система золотого стандарта, нанокомпозит с циркониевым наполнителем и 10+ лет непрерывного постдипломного образования врача.\n\n"
            "💬 <b>Скрипт для врача:</b>\n"
            "<i>«Вы платите не за свет синей лампы, а за то, чтобы восстановленный зуб служил вам 10–15 лет, сохранил прикус и живой нерв. "
            "Мы работаем в стерильных условиях изоляции, микроборами под увеличением и сложными наноматериалами, которые воспроизводят прочность натуральной эмали».</i>\n\n"
            "😄 <b>Врачебная жиза:</b>\n"
            "<i>«Посветить фонариком» стоит 0 рублей. А вот знать, куда посветить, под каким углом, каким композитом и как при этом не убить пульпу — стоит ровно столько, сколько написано в плане лечения.</i>"
        )
    },
    "ultrasound_enamel": {
        "title": "🪨 «Ультразвук сдирает эмаль»",
        "text": (
            "🗣 <b>Пациент:</b> <i>«Мне ультразвук не делайте, он всю эмаль сдирает до дыр! Вы мне просто пастой и щеточкой помойте»</i>\n\n"
            "📋 <b>Клинический декодер (МКБ-10):</b>\n"
            "• Диагноз: <b>K03.6 Зубные отложения</b> (поддесневой и наддесневой зубной камень).\n"
            "• EBM-реальность: Твердость эмали по шкале Мооса равна 5 (сопоставимо с апатитом и сталью), а минерализованного зубного камня — всего 2.5–3. Насадка ультразвукового скейлера колеблется с частотой 25 000–30 000 Гц и при правильном угле (параллельно поверхности зуба) разрушает только хрупкую кристаллическую решетку камня эффектом кавитации, не повреждая эмаль.\n\n"
            "💬 <b>Скрипт для врача:</b>\n"
            "<i>«Ультразвуковая насадка не срезает зуб, как бор. Она вибрирует микроскопическими волнами в водяном облаке, разрушая только плотный зубной камень, как сосульку с крыши. "
            "Эмаль зуба в разы прочнее камня. А вот если камень оставить, под ним десна будет разрушаться дальше».</i>\n\n"
            "😄 <b>Врачебная жиза:</b>\n"
            "<i>Пациенты считают 4 мм твердого минерализованного поддесневого камня естественной броней зуба, а кавитационный скейлер — отбойным молотком шахтера.</i>"
        )
    },
    "crown_superglue": {
        "title": "🧴 «Приклейте старую коронку на клей Момент»",
        "text": (
            "🗣 <b>Пациент:</b> <i>«У меня слетела старая коронка, которой 15 лет. Не надо ничего переделывать, просто посадите её обратно на суперклей Момент!»</i>\n\n"
            "📋 <b>Клинический декодер (МКБ-10):</b>\n"
            "• Диагноз: <b>K08.8 Расцементировка ортопедической конструкции</b> / Вторичный кариес культи зуба.\n"
            "• EBM-реальность: Коронка слетает не потому, что «цемент кончился», а из-за вторичного кариеса, разрушившего культю зуба под ней. Цианакрилат (суперклей) цитотоксичен, вызывает химический некроз десны и периодонтит, а коронка без культи сорвется во время еды и попадет в трахею.\n\n"
            "💬 <b>Скрипт для врача:</b>\n"
            "<i>«Коронка слетела из-за того, что под ней изменилась форма зуба из-за скрытого разрушения. "
            "Технический клей токсичен для живых тканей и разрушит корень за неделю. Давайте сделаем рентген, оценим состояние корня и зафиксируем коронку на безопасный медицинский цемент, если зуб сохранил опору».</i>\n\n"
            "😄 <b>Врачебная жиза:</b>\n"
            "<i>Суперклей намертво склеивает только пальцы хирурга и наивные мечты пациента о вечном протезировании за 50 рублей.</i>"
        )
    }
}

MATERIAL_BATTLE_CARDS = {
    "ceramics": {
        "title": "👑 Цирконий (ZrO2) vs Дисиликат лития (E.max)",
        "text": (
            "👑 <b>Material Battle: Диоксид циркония (ZrO2) vs Дисиликат лития (E.max)</b>\n\n"
            "🔬 <b>1. Механические свойства и физика:</b>\n"
            "• <b>ZrO2 (3Y/4Y/5Y-TZP):</b> Прочность на изгиб <b>800–1200 МПа</b> (3Y) / <b>600–800 МПа</b> (4Y Multi). Устойчив к катастрофическим сколам при бруксизме. Требует меньшей толщины препарирования (от 0.6–0.8 мм).\n"
            "• <b>E.max (Дисиликат лития):</b> Прочность на изгиб <b>400–500 МПа</b>. Непревзойденная флуоресценция и транслюцентность. Минимальная толщина 1.0–1.2 мм.\n\n"
            "🧪 <b>2. Протокол фиксации (Критическая разница!):</b>\n"
            "• <b>E.max (Травление плавиковой кислотой):</b> Травление 5% плавиковой кислотой (HF) <b>20 секунд</b> → промывание водой / УЗ-очистка спиртом → нанесение силана (Monobond Plus) → адгезивная фиксация на композитный цемент светового/двойного отверждения (Variolink Esthetic).\n"
            "• <b>Цирконий (Пескоструй + 10-MDP):</b> Травление HF не работает (нет стеклофазы)! Пескоструй оксидом алюминия (Al2O3 50 мкм, 1.5–2 бар) → очистка Ivoclean → праймер с <b>10-MDP</b> (Z-Prime Plus / Monobond Plus) → самоадгезивный цемент (RelyX U200 / Panavia V5).\n\n"
            "🏆 <b>Вердикт:</b>\n"
            "• <i>E.max</i> — абсолютный чемпион для виниров, вкладок Inlay/Onlay/Overlay и одиночных коронок фронтальной группы.\n"
            "• <i>Цирконий</i> — выбор номер 1 для мостовидных протезов любой протяженности, коронок на имплантатах и жевательных зубов при бруксизме."
        )
    },
    "adhesion": {
        "title": "💧 OptiBond FL vs Universal Adhesives",
        "text": (
            "💧 <b>Material Battle: OptiBond FL (4th Gen) vs Universal Adhesives (8th Gen)</b>\n\n"
            "🔬 <b>1. Физика адгезивного шва и сила сцепления:</b>\n"
            "• <b>OptiBond FL (3 шага: кислота + праймер + бонд):</b> Золотой стандарт EBM уже более 25 лет. Сила сцепления с дентином <b>35–42 МПа</b>. Толстая гидрофобная смола бонда создает плотный эластичный буферный гибридный слой, гасящий полимеризационный стресс.\n"
            "• <b>Universal (Single Bond Universal, G-Premio Bond):</b> Однобутылочная система. Сила сцепления с травленым дентином <b>25–30 МПа</b>. Ультратонкая пленка (5–10 мкм), содержит гидрофильные мономеры (HEMA), подвержена гидролитической деградации со временем.\n\n"
            "🧪 <b>2. Особенности протокола:</b>\n"
            "• OptiBond FL требует тотального протравливания (Total-Etch: эмаль 30 сек, дентин строго 15 сек) и раздельного нанесения праймера и смолы. Чувствителен к пересушиванию дентина.\n"
            "• Universal работает в режимах: селективное травление эмали (Selective Etch — лучший выбор), Total-Etch и Self-Etch. Содержит мономер <b>10-MDP</b> для химической связи с гидроксиапатитом.\n\n"
            "🏆 <b>Вердикт:</b>\n"
            "• Для непрямых реставраций (IDS — немедленное запечатывание дентина) и высоконагруженных полостей II класса OptiBond FL превосходит конкурентов по долговечности.\n"
            "• Универсальные бонды выигрывают по скорости и универсальности при селективном травлении эмали."
        )
    },
    "sealer": {
        "title": "🔬 Биокерамика (BioRoot RCS) vs AH Plus",
        "text": (
            "🔬 <b>Material Battle: Биокерамика (BioRoot RCS) vs Эпоксидная смола (AH Plus)</b>\n\n"
            "🔬 <b>1. Химическая природа и поведение в канале:</b>\n"
            "• <b>AH Plus (Dentsply Sirona):</b> Эпоксидно-аминная смола. Золотой стандарт обтурации с 1995 года. Нулевая растворимость, минимальная усадка (0.1%), рентгеноконтрастность. Не обладает собственной биоактивностью, инертен.\n"
            "• <b>BioRoot RCS (Septodont):</b> Гидравлический силикат кальция высокой чистоты. Биоактивен: высвобождает ионы Ca(OH)2, стимулирует образование кристаллов гидроксиапатита на границе с дентином. Высокощелочной pH (11.7+), мощный антибактериальный эффект.\n\n"
            "🧪 <b>2. Методика обтурации:</b>\n"
            "• AH Plus идеален для термопластической конденсации гуттаперчи (метод непрерывной волны Скилдера / гутта-пак). Термостабилен до 200°C.\n"
            "• Биокерамика не любит нагрев выше 100°C (теряет кристаллизационную воду)! Применяется методом <b>одиночного калиброванного штифта (Single-Cone Hydraulic)</b> на холодную.\n\n"
            "⚠️ <b>3. Распломбировка (Перелечивание):</b>\n"
            "• AH Plus легко размягчается растворителями (эвкалиптол, хлороформ) и ультразвуком.\n"
            "• Затвердевшая биокерамика в апикальной трети формирует структуру типа бетона, распломбировывается со значительными механическими трудностями.\n\n"
            "🏆 <b>Вердикт:</b>\n"
            "• <i>AH Plus + горячая гуттаперча</i> — лучший выбор для каналов сложной анатомии, овальных каналов и дельтовидных разветвлений.\n"
            "• <i>BioRoot RCS / BC Sealer</i> — метод выбора при апикальных периодонтитах, широких апикальных отверстиях и холодной гидравлической обтурации."
        )
    },
    "mta": {
        "title": "🧱 MTA (ProRoot) vs Biodentine",
        "text": (
            "🧱 <b>Material Battle: MTA (ProRoot MTA) vs Biodentine (Septodont)</b>\n\n"
            "🔬 <b>1. Состав и время твердения:</b>\n"
            "• <b>MTA (Минерал Триоксид Агрегат):</b> Портландцемент модифицированный + оксид висмута (радиопак). Время полного первичного отверждения — <b>2.5–4 часа</b>. Требует покрытия влажным шариком и повторного визита для постоянной реставрации.\n"
            "• <b>Biodentine («Искусственный дентин»):</b> Синтетический чистый трикальцийсиликат + оксид циркония. Время отверждения — всего <b>12 минут</b>! Позволяет завершить прямую композитную реставрацию в то же посещение.\n\n"
            "🎨 <b>2. Изменение цвета зуба (Дисколорит):</b>\n"
            "• Оксид висмута в классическом сером и белом MTA при контакте с NaOCl или кровью окисляется и дает темное серо-зеленое окрашивание коронки (противопоказан в зоне улыбки при витальной ампутации!).\n"
            "• Biodentine не содержит соединений висмута, не вызывает дисколорита тканей зуба, эстетически полностью безопасен.\n\n"
            "💪 <b>3. Прочность и физические свойства:</b>\n"
            "• Прочность на сжатие Biodentine достигает <b>300 МПа</b> (сопоставимо с естественным дентином), модуль упругости 22 ГПа.\n"
            "• MTA имеет меньшую прочность на сжатие (≈ 70 МПа) и зернистую песчаную консистенцию.\n\n"
            "🏆 <b>Вердикт:</b>\n"
            "• <i>Biodentine</i> — бесспорный лидер для прямого/непрямого покрытия пульпы (Direct Pulp Capping), глубоких пришеечных полостей и перфораций дна полости зуба за 1 визит.\n"
            "• <i>MTA</i> — традиционный надежный выбор для апексификации и ретроградного пломбирования при резекции верхушки корня."
        )
    },
    "bopt": {
        "title": "📐 BOPT (без уступа) vs Уступ Chamfer / Shoulder",
        "text": (
            "📐 <b>Material Battle: BOPT (вертикальное препарирование) vs Классический уступ</b>\n\n"
            "🔬 <b>1. Суть концепций:</b>\n"
            "• <b>BOPT (Biologically Oriented Preparation Technique):</b> Безуступное препарирование пламевидным бором с намеренным кюретажем зубодесневой борозды (гингивоттомия). Цель: стереть старую эмалево-цементную границу и сформировать новую толстую десневую манжетку вокруг коронки.\n"
            "• <b>Уступ (Chamfer / Shoulder):</b> Четкая граница препарирования шириной 0.8–1.2 мм с закругленным внутренним углом. Абсолютный визуальный контроль для зубного техника.\n\n"
            "⚖️ <b>2. Сравнение преимуществ и рисков:</b>\n"
            "• <i>Плюсы BOPT:</i> Идеально для зубов с рецессией десны, пародонтитом, старыми дефектами уступов. Утолщает тонкий биотип десны на 30–50%, устраняет синюшность края.\n"
            "• <i>Минусы BOPT:</i> Высокая зависимость от мастерства техника (риск нависающего края и воспаления десны), требует обязательного ношения временных коронок не менее 4–6 недель для созревания десны.\n"
            "• <i>Плюсы уступа:</i> Идеальная лабораторная посадка, минимальный риск затеков цемента, стандарт E.max реставраций.\n\n"
            "🏆 <b>Вердикт:</b>\n"
            "• <i>BOPT</i> незаменим в сложных переделках, при повторном протезировании с разрушенными уступами и тонком биотипе десны.\n"
            "• <i>Уступ Chamfer</i> остается золотым стандартом первичного протезирования керамическими коронками и винирами."
        )
    },
    "post": {
        "title": "🔩 Культевая вкладка vs СВШ vs Build-Up без штифта",
        "text": (
            "🔩 <b>Material Battle: Литая вкладка vs Стекловолоконный штифт (СВШ) vs Build-Up</b>\n\n"
            "🔬 <b>1. Модуль упругости и биомеханика раскола:</b>\n"
            "• <b>Литая вкладка (КХС / Золото):</b> Модуль упругости металла <b>200 ГПа</b> (в 10 раз жестче дентина 18 ГПа!). Металл не амортизирует жевательную нагрузку, а работает как клин, передавая вектор силы на стенки корня. Главная причина катастрофических продольных переломов корня зуба.\n"
            "• <b>СВШ (Стекловолоконный штифт):</b> Модуль упругости <b>15–20 ГПа</b> — идентичен натуральному дентину! Штифт изгибается синхронно с зубом, гася полимеризационный стресс.\n"
            "• <b>Build-Up без штифта:</b> Монолитная композитная культя (Core-композит двойного отверждения), связанная с дентином адгезивом.\n\n"
            "📏 <b>2. Правило Феррула (Ferrule Effect) — Решающий фактор:</b>\n"
            "• Если сохранены стенки зуба высотой <b>≥ 1.5–2.0 мм</b> и толщиной ≥ 1.0 мм по всему периметру (Феррул есть) — <b>штифт не нужен вообще</b>! Build-Up на адгезиве служит так же надежно, как со штифтом.\n"
            "• Если феррула нет (разрушение под десну) — никакой штифт зуб не спасет без хирургического удлинения коронковой части или экструзии.\n\n"
            "🏆 <b>Вердикт:</b>\n"
            "• Литые металлические вкладки — устаревший опасный метод в 95% случаев.\n"
            "• Выбор современной клиники: <i>Build-Up на СВШ</i> при дефиците культи; <i>Build-Up без штифта</i> при сохранном ферруле."
        )
    },
    "implant_retention": {
        "title": "🔩 Винтовая vs Цементная фиксация на имплантатах",
        "text": (
            "🔩 <b>Material Battle: Винтовая фиксация vs Цементная фиксация на имплантатах</b>\n\n"
            "🔬 <b>1. Главный враг имплантации — цементный периимплантит:</b>\n"
            "При цементной фиксации излишки цемента неизбежно выдавливаются под десну. В 80% случаев поддесневой цемент невозможно убрать даже ультразвуком. Он обрастает биопленкой, вызывая резорбцию кости вокруг шейки имплантата.\n\n"
            "⚖️ <b>2. Сравнение характеристик:</b>\n"
            "• <b>Винтовая фиксация (Золотой стандарт EBM):</b>\n"
            "  — <i>Плюсы:</i> 100% ремонтопригодность (открутил за 1 минуту для полировки, замены винта или ревизии десны), ноль риска попадания цемента под десну.\n"
            "  — <i>Минусы:</i> Шахта винта выходит на окклюзионную поверхность (требует закрытия композитом); при наклоне имплантата во фронте шахта может выйти на вестибулярную сторону (решается угловыми шахтами ASC до 25–30°).\n"
            "• <b>Цементная фиксация:</b>\n"
            "  — <i>Плюсы:</i> Пассивность посадки, отсутствие шахты винта на видимой эмали.\n"
            "  — <i>Минусы:</i> Невозможно снять коронку без распиливания, высокий риск потери кости.\n\n"
            "🏆 <b>Вердикт:</b>\n"
            "Винтовая фиксация — безоговорочный стандарт современной имплантологии. Цементная фиксация допустима только на индивидуальных титановых абатментах с супрагингивальным уступом (уступ выше десны на 0.5 мм)."
        )
    },
    "airflow": {
        "title": "💨 Порошки Air-Flow: Сода vs Глицин vs Эритритол",
        "text": (
            "💨 <b>Material Battle: Порошки для воздушно-абразивной обработки (Air-Flow)</b>\n\n"
            "🔬 <b>1. Размер и форма гранул (Абразивность):</b>\n"
            "• <b>Бикарбонат натрия (Пищевая сода, 40–65 мкм):</b> Остроугольные кристаллы. Сдирает не только налет, но и полировку пломб, оставляет глубокие микроцарапины на эмали, агрессивно разрушает обнаженный дентин и цемент корня.\n"
            "• <b>Глицин (Аминокислота, 20–25 мкм):</b> Мягкий водорастворимый порошок со сладковатым вкусом. Безопасен для маргинальной десны и мягких тканей.\n"
            "• <b>Эритритол (Многоатомный спирт, 14 мкм):</b> Ультрамелкий порошок наивысшей безопасности. Имеет антибактериальный эффект.\n"
            "• <b>Оксид алюминия (Al2O3 27/50 мкм):</b> Это НЕ порошок для чистки! Это микропескоструй для подготовки керамики, циркония и металла перед бондингом (Rondoflex / CoJet).\n\n"
            "🎯 <b>2. Клинические показания:</b>\n"
            "• <i>Сода:</i> Только грубый наддесневой налет курильщика на интактной эмали. Категорически запрещена в поддесневой зоне и на имплантатах!\n"
            "• <i>Глицин / Эритритол (протокол GBT):</i> Чистка поддесневых пародонтальных карманов, брекет-систем, композитных реставраций и поверхности дентальных имплантатов без риска повреждения остеоинтеграции.\n\n"
            "🏆 <b>Вердикт:</b>\n"
    ...
    "isolation": {
        ...
        "text": (
            "🛡 <b>Material Battle: Коффердам с кламмерами vs OptiDam vs Валики</b>\n\n"
            "🔬 <b>1. Физика адгезивного шва и слюна:</b>\n"
            "Даже микроскопическая капля слюны или влажное дыхание пациента на протравленный дентин снижает силу адгезии с 35–40 МПа до <b>3–5 МПа</b> (падение на 90%!). Никакие ватные валики и слюноотсос не способны блокировать влажность выдыхаемого воздуха (100% влажность).\n\n"
            ...
            "  — 100% защита пациента от проглатывания/аспирации боров, эндофайлов и ожога гипохлоритом.\n"
            "  — Расслабление жевательных мышц пациента и экономия времени врача до 30%.\n"
            "• <b>OptiDam (3D-форма Kerr):</b> Удобен для новичков, быстрая установка, но ограничен при глубоких поддесневых дефектах II класса.\n"
            "• <b>Ватные валики:</b> Имитация изоляции. Приемлемы только для снятия швов или осмотра.\n\n"
            "🏆 <b>Вердикт:</b>\n"
            "Эндодонтия и прямые композитные реставрации без коффердама по современным стандартам EBM приравниваются к заведомому клиническому браку."
        )
    },
    "gi_composite": {
        "title": "👶 СИЦ (Fuji IX) vs Композит в детской стоматологии",
        "text": (
            "👶 <b>Material Battle: Стеклоиономер (СИЦ) vs Композит у детей</b>\n\n"
            "🔬 <b>1. Химическая адгезия vs Влагобоязнь:</b>\n"
            "• <b>СИЦ высокой вязкости (Fuji IX, Ketac Molar):</b> Вступает в истинную химическую связь (хелатные связи с кальцием дентина и эмали). Не боится остаточной влажности! Выделяет ионы фтора (эффект перезарядки), подавляя вторичный кариес.\n"
            "• <b>Композит (Filtek, Estelite):</b> Требует безупречной сухости (коффердам), травления и бондинга. При попадании хотя бы микрокапли слюны у неконтактного ребенка — реставрация выпадет через 3 месяца с рецидивом кариеса.\n\n"
            "🎯 <b>2. Тактика по возрасту и поведению:</b>\n"
            "• <i>Неконтактный ребенок (до 3–4 лет), множественный кариес:</i> СИЦ без бормашины (техника ART) — единственный способ спасти зубы от удаления без наркоза.\n"
            "• <i>Контактный ребенок с коффердамом (от 5 лет):</i> Композит превосходит СИЦ по истираемости и эстетике.\n\n"
            "🏆 <b>Вердикт:</b>\n"
            "• В условиях дефицита времени и влаги — <i>Fuji IX</i> спасает зуб.\n"
            "• При стабильной изоляции — <i>композит</i> обеспечивает анатомическую форму и долговечность."
        )
    }
}
...
"""
        ...
        if (text.lower() == "/record" or text.lower().startswith("/record ") or
            ...
            if not record_arg:
                ...
                from telethon import Button
                buttons = [
                    [Button.inline("🦷 Терапия (Кариес/Пульпит)", data="record:therapy"), Button.inline("👑 Ортопедия (Коронка/BOPT)", data="record:ortho")],
                    [Button.inline("🔪 Хирургия (Удаление/Имплант)", data="record:surgery"), Button.inline("🩸 Пародонтология (SRP)", data="record:perio")],
                    [Button.inline("⬅️ В главное меню", data="nav:main")]
                ]
        ...
        if (text.lower() == "/rx" or text.lower().startswith("/rx ") or
            ...
            if not rx_arg:
                ...
                from telethon import Button
                buttons = [
                    [Button.inline("🦴 Бисфосфонаты (MRONJ)", data="rx:mronj"), Button.inline("🩸 Антикоагулянты (ПОАК/МНО)", data="rx:anticoag")],
                    [Button.inline("❤️ Кардиориски и адреналин", data="rx:cardio"), Button.inline("🛡 Профилактика эндокардита", data="rx:endo")],
                    [Button.inline("⬅️ В главное меню", data="nav:main")]
                ]
        ...
        if (text.lower() == "/concilium" or text.lower().startswith("/concilium ") or
            ...
            if not conc_arg:
                ...
                from telethon import Button
                buttons = [
                    [Button.inline("📋 Демонстрационный консилиум", data="concilium:example")],
                    [Button.inline("⬅️ В главное меню", data="nav:main")]
                ]
        ...
        if (text.lower() == "/sos" or text.lower().startswith("/sos ") or
            ...
            if not sos_arg:
                ...
                buttons = [
                    ...
                    [Button.inline("💔 Файлолом (Сломан файл)", data="sos:file"), Button.inline("🕳 Перфорация (MTA/Биодентин)", data="sos:perf")],
                    [Button.inline("⚠️ Выведение за апекс", data="sos:sealer"), Button.inline("🩸 Кровотечение из лунки", data="sos:bleed")],
                    [Button.inline("🫁 Аспирация предмета", data="sos:aspiration"), Button.inline("⚡️ Не берет анестезия", data="sos:anesthesia_failure")],
                    [Button.inline("💨 Эмфизема мягких тканей", data="sos:emphysema"), Button.inline("🦴 Перфорация пазухи", data="sos:sinus_perf")],
                    [Button.inline("⬅️ В главное меню", data="nav:main")]
                ]
        ...
        if (text.lower() == "/translate" or text.lower().startswith("/translate ") or
            ...
            if not trans_arg:
                ...
                buttons = [
                    ...
                    [Button.inline("☠️ «Положите мышьяк»", data="trans:arsenic"), Button.inline("⚡️ «Пломба лазером без бормашины»", data="trans:laser")],
                    [Button.inline("🦴 «Кость рассосалась»", data="trans:bone"), Button.inline("❄️ «Нерв в десне простудил»", data="trans:nerve")],
                    [Button.inline("🍼 «Ребенок кальций высосал»", data="trans:calcium"), Button.inline("👶 «Зачем молочный лечить»", data="trans:milk_teeth")],
        ...
        if (text.lower() == "/vs" or text.lower().startswith("/vs ") or
            ...
            if not vs_arg:
                ...
                buttons = [
                    ...
                    [Button.inline("📐 BOPT vs Уступ Chamfer", data="vs:bopt"), Button.inline("🔩 Вкладка vs СВШ", data="vs:post")],
...
async def handle_quiz_callback(bot_client, event):
    ...
    if data_str.startswith("nav:") or data_str.startswith("menu:"):
        ...
        if nav_target in ("main", "home"):
            ...
            buttons = [
                ...
                [Button.inline("⚠️ Выведение за апекс", data="sos:sealer"), Button.inline("🩸 Кровотечение", data="sos:bleed")],
                [Button.inline("🫁 Аспирация предмета", data="sos:aspiration"), Button.inline("⚡️ Не берет анестезия", data="sos:anesthesia_failure")],
    ...
    if data_str.startswith("record:"):
        ...
        back_btns = [
            [Button.inline("📋 К списку шаблонов", data="nav:record")],
            [Button.inline("⬅️ В главное меню", data="nav:main")]
        ]
        if rec_kind == "therapy":
            therapy_card = (
                "📋 <b>Шаблон 043/у: Терапевтический приём (Глубокий кариес)</b>\n\n"
                "<b>Зуб:</b> 4.6\n"
                "<b>Диагноз (МКБ-10):</b> K02.1 Кариес дентина (глубокий кариозный процесс).\n"
                "<b>Жалобы:</b> Кратковременные боли от температурных (холодное) и химических (сладкое) раздражителей, быстро проходящие после устранения фактора; застревание пищи.\n"
                "<b>Анамнез:</b> Ранее зуб не лечен. Соматически здоров, аллергоанамнез не отягощен.\n"
                "<b>Status praesens:</b> На жевательной и медиальной поверхности 4.6 глубокая кариозная полость, заполненная пигментированным размягченным дентином. Зондирование болезненно по эмалево-дентинной границе. Термометрия (холод): кратковременная быстропроходящая боль. Перкуссия безболезненна. ЭОД: 6 мкА. Рентгенография: дефект твердых тканей в пределах околопульпарного дентина, периапикальных изменений нет.\n\n"
                "<b>Протокол лечения:</b>\n"
                "1. Проводниковая торусальная и инфильтрационная анестезия Sol. Articaini 4% (1:200 000) 1.7 мл.\n"
                "2. Изоляция операционного поля системой коффердам (кламп W8A Sanctuary).\n"
                "3. Препарирование твердых тканей, поэтапная некрэктомия под контролем кариес-маркера (Seek). Формирование полости.\n"
                "4. Антисептическая обработка Sol. Chlorhexidini 2.0%.\n"
                "5. Адгезивный протокол: самопротравливающий праймер + бонд OptiBond FL (IV поколение), полимеризация 20 сек.\n"
                "6. Восстановление анатомической формы по буграм наногибридным композитом Ceram.X SphereTEC A3 с моделировкой фиссур I порядка.\n"
                "7. Фотополимеризация под слоем глицеринового геля DeOx (ингибирование кислородного слоя).\n"
                "8. Окклюзионная интеграция под контролем копирки Bausch 40 и 12 мкм. Шлифовка и финишная полировка головками Enhance и пастой Prisma Gloss.\n\n"
                "<b>Рекомендации:</b> Щадящая жевательная нагрузка 24 часа. Контрольный осмотр через 6 месяцев."
            )
            await edit_callback_message(bot_client, event, therapy_card, "edit_message:record_therapy", buttons=back_btns, parse_mode='html')
            await event.answer()
            return

        elif rec_kind == "ortho":
            ortho_card = (
                "📋 <b>Шаблон 043/у: Ортопедический приём (Препарирование под коронку)</b>\n\n"
                "<b>Зуб:</b> 1.6\n"
                "<b>Диагноз (МКБ-10):</b> K08.8 Другие уточненные изменения зубов (ИРОПЗ > 0.8, дефект твердых тканей короночной части зуба).\n"
                "<b>Жалобы:</b> На скол коронковой части зуба, нарушение жевания и эстетики.\n"
                "<b>Status praesens:</b> Коронковая часть 1.6 разрушена более чем на 80%. Корневые каналы обтурированы до верхушек плотно, периапикальный периодонт интактен. Культя восстановлена композитным материалом Core-it на стекловолоконном штифте с сохранением феррула 2.0 мм по всему периметру.\n\n"
                "<b>Протокол приёма:</b>\n"
                "1. Инфильтрационная анестезия Sol. Articaini 4% (1:200 000) 1.7 мл.\n"
                "2. Одонтопрепарирование зуба 1.6 под цельноанатомическую коронку из диоксида циркония с созданием кругового желобовидного уступа (chamfer) 0.8 мм на уровне десневого края. Контроль конвергенции стенок 6°.\n"
                "3. Механическая ретракция десны нитью Ultrapak 00 с гемостатиком ViscoStat Clear.\n"
                "4. Снятие двухфазного прецизионного оттиска А-силиконовой массой (Honigum Putty + Light) / интраоральное 3D-сканирование.\n"
                "5. Регистрация центральной окклюзии силиконовым регистратором Futar D.\n"
                "6. Изготовление провизорной коронки прямым методом материалом Protemp 4, фиксация на безевгенольный цемент Temp-Bond NE.\n\n"
                "<b>Рекомендации:</b> Аккуратная гигиена флоссом в зоне временной конструкции. Следующий визит — примерка и фиксация."
            )
            await edit_callback_message(bot_client, event, ortho_card, "edit_message:record_ortho", buttons=back_btns, parse_mode='html')
            await event.answer()
            return

        elif rec_kind == "surgery":
            surgery_card = (
                "📋 <b>Шаблон 043/у: Хирургический приём (Атипичное удаление ретинированного 3.8)</b>\n\n"
                "<b>Зуб:</b> 3.8\n"
                "<b>Диагноз (МКБ-10):</b> K01.1 Ретинированные зубы (мезиоангулярная ретенция и дистопия 3.8).\n"
                "<b>Жалобы:</b> Периодические тянущие боли в области угла нижней челюсти слева с иррадиацией в ухо, травматизация щеки.\n"
                "<b>Status praesens:</b> Зуб 3.8 погружен в костную ткань на 2/3 коронки, мезиальные бугры упираются в дистальный корень 3.7. Слизистый капюшон гиперемирован. На ОПТГ/КЛКТ: корни искривлены, верхушки в непосредственной близости к нижнечелюстному каналу (без инвазии).\n\n"
                "<b>Протокол операции:</b>\n"
                "1. Торусальная и щечная анестезия Sol. Articaini 4% (1:100 000) 3.4 мл. Аспирационная проба отрицательна.\n"
                "2. Угловой разрез слизистой от ветви челюсти к дистальной поверхности 3.7 с внутрибороздовым продолжением. Отслаивание слизисто-надкостничного лоскута.\n"
                "3. Трепанация кортикальной пластинки хирургическим фиссурным бором под обильным охлаждением стерильным Sol. Natrii chloridi 0.9%.\n"
                ...
                "6. Кюретаж лунки, ревизия дна, удаление фолликулярного мешочка, сглаживание острых костных краев фрезой.\n"
                "7. Гемостаз: внесение коллагеновой губки с гентамицином / йодоформной турунды. Мобилизация лоскута.\n"
                "8. Наложение 3 узловых швов шовным материалом Vicryl 4-0.\n\n"
                "<b>Назначения:</b> Местно холод 15 мин x 3 раза. Нимесил 100 мг при болях (до 3 дней). Ротовые ванночки хлоргексидин 0.12% с 2-х суток (не полоскать!). Снятие швов через 7-10 дней."
            )
            await edit_callback_message(bot_client, event, surgery_card, "edit_message:record_surgery", buttons=back_btns, parse_mode='html')
            await event.answer()
            return

        elif rec_kind == "perio":
            perio_card = (
                "📋 <b>Шаблон 043/у: Пародонтологический приём (SRP — Scaling and Root Planing)</b>\n\n"
                "<b>Область:</b> Секстант 4 (зубы 4.4, 4.5, 4.6, 4.7)\n"
                "<b>Диагноз (МКБ-10):</b> K05.3 Хронический генерализованный пародонтит средней степени тяжести в стадии обострения.\n"
                "<b>Жалобы:</b> Кровоточивость десен при чистке зубов и приеме твердой пищи, неприятный запах изо рта, подвижность зубов.\n"
                "<b>Status praesens:</b> Маргинальная и сосочковая десна гиперемирована, отечна, цианотична. Глубина пародонтальных карманов 4-5 мм. Индекс кровоточивости BOP 68%. Поддесневой зубной камень обильный. Подвижность зубов I степени по Миллеру.\n\n"
                "<b>Протокол лечения:</b>\n"
                "1. Аппликационная анестезия гелем Лидокаин 5% + инфильтрационная анестезия Sol. Articaini 4% (1:200 000) 1.7 мл.\n"
                "2. Ультразвуковой поддесневой скейлинг с антисептическим орошением Sol. Chlorhexidini 0.12% тонкой пародонтологической насадкой Piezo.\n"
                "3. Закрытый кюретаж и сглаживание поверхности корней (Root Planing) зоноспецифическими кюретами Грейси (Gracey 11/12, 13/14). Удаление грануляций и биопленки.\n"
                "4. Деконтаминация карманов низкоинтенсивным диодным лазером / ирригация Sol. Chlorhexidini 0.2%.\n"
                "5. Аппликация адгезивного дентального геля Метрогил Дента под десневую повязку Septo-pack.\n\n"
                "<b>Рекомендации:</b> Щетка ультрамягкая (Curaprox 5460), межзубные ершики подходящего калибра, ополаскиватель 0.12% хлоргексидин 7 дней. Повторный осмотр и повторное зондирование карманов через 4-6 недель."
            )
            await edit_callback_message(bot_client, event, perio_card, "edit_message:record_perio", buttons=back_btns, parse_mode='html')
            await event.answer()
            return
    ...
    if data_str.startswith("rx:"):
        ...
        rx_back_btns = [
            [Button.inline("🛡 К разделам рисков", data="nav:rx")],
            [Button.inline("⬅️ В главное меню", data="nav:main")]
        ]
        if rx_kind == "mronj":
            mronj_card = (
                "🦴 <b>EBM-Гайдлайн: Бисфосфонаты и остеонекроз челюстей (MRONJ)</b>\n\n"
                "<b>Препараты:</b> Золедронат (Зомета), Алендронат (Фосамакс), Ибандронат (Бонвива), Деносумаб (Пролиа, Эксджива).\n\n"
                "<b>Клиническая стратификация риска:</b>\n"
                "• <b>Высокий риск (5–15%):</b> Внутривенные бисфосфонаты и таргетная антирезорбтивная терапия в онкологии (миелома, метастазы в кости). Плановые удаления, имплантация и костная пластика <b>абсолютно противопоказаны</b>!\n"
                "• <b>Низкий риск (< 0.1%):</b> Пероральный приём при остеопорозе длительностью менее 3-4 лет при отсутствии системных факторов (ГКС, диабет, курение).\n"
                "• <b>Умеренный риск (0.5–1%):</b> Пероральный приём > 4 лет либо сочетание с приёмом кортикостероидов (преднизолон).\n\n"
                "<b>Лабораторный маркер:</b>\n"
                "• Тест на сывороточный С-концевой телопептид (s-CTX): > 150 пг/мл — безопасная зона; 100–150 пг/мл — зона умеренного риска; < 100 пг/мл — критический риск.\n\n"
                "<b>Хирургический протокол при вынужденном удалении:</b>\n"
                "1. Антибиотикопрофилактика: Амоксиклав 1000 мг 2 раза в день за 24 часа до и 10–14 дней после операции.\n"
            ...
            await edit_callback_message(bot_client, event, mronj_card, "edit_message:rx_mronj", buttons=rx_back_btns, parse_mode='html')
            await event.answer()
            return

        elif rx_kind == "anticoag":
            anticoag_card = (
                "🩸 <b>EBM-Гайдлайн: Антикоагулянты и антиагреганты в стоматологии</b>\n\n"
                "<b>1. Прямые оральные антикоагулянты — ПОАК/НОАК (Ксарелто, Эликвис, Прадакса):</b>\n"
                "• <b>НЕЛЬЗЯ отменять самостоятельно!</b> Риск фатального тромбоэмболического инсульта несоизмеримо выше риска местного луночкового кровотечения.\n"
                "• Вмешательство проводят через 12–24 часа после приема последней дозы (на минимальной концентрации препарата в плазме).\n\n"
                "<b>2. Антагонисты витамина К (Варфарин):</b>\n"
                "• Экспресс-анализ МНО (INR) за 24–48 часов до вмешательства.\n"
                "• Безопасный коридор для рутинной хирургии: <b>МНО 2.0 – 3.0</b>. При МНО < 3.5 рутинное удаление зубов и имплантация проводятся <b>без отмены Варфарина</b>!\n\n"
                "<b>3. Антиагреганты (Аспирин 75–100 мг, Тромбо АСС, Плавикс/Клопидогрел):</b>\n"
                "• Монотерапия аспирином НЕ отменяется.\n"
                "• Двойная антитромбоцитарная терапия (ДАТТ после стентирования) — отмена строго запрещена кардиологами первые 6–12 месяцев.\n\n"
                "<b>Протокол локального гемостаза:</b>\n"
                "• Плотное тампонирование лунки коллагеновым конусом / гемостатической губкой с тромбином.\n"
                "• Обязательное ушивание лунки крестообразным или узловым швом (Vicryl 4-0).\n"
                "• Местная аппликация марлевого тампона с 5% раствором транексамовой кислоты на 30 мин. Назначение ротовых ванночек с транексамом (без активного полоскания!)."
            )
            await edit_callback_message(bot_client, event, anticoag_card, "edit_message:rx_anticoag", buttons=rx_back_btns, parse_mode='html')
            await event.answer()
            return

        elif rx_kind == "cardio":
            cardio_card = (
                "❤️ <b>EBM-Гайдлайн: Кардиологические риски и выбор вазоконстрикторов</b>\n\n"
                "<b>Артериальная гипертензия — классификация для приёма:</b>\n"
                "• <b>АД < 140/90 мм рт. ст.:</b> Стандартный приём, адреналин 1:100 000 или 1:200 000 без ограничений.\n"
                "• <b>АД 140–159 / 90–99 мм рт. ст.:</b> Умеренный риск. Максимум 2 карпулы с адреналином 1:200 000. Обязательна аспирационная проба!\n"
                "• <b>АД 160–179 / 100–109 мм рт. ст.:</b> Высокий риск. Только экстренная помощь. Исключить адреналин! Препарат выбора: <b>Мепивакаин 3% без вазоконстриктора</b> (до 2–3 карпул).\n"
                "• <b>АД ≥ 180/110 мм рт. ст.:</b> Стоматологическое лечение абсолютно противопоказано. Купирование криза (Каптоприл 25 мг под язык), при загрудинных болях — скорая медицинская помощь.\n\n"
                "<b>ИБС, аритмии, постинфарктный период:</b>\n"
                "• Первые 6 месяцев после перенесенного инфаркта миокарда или ОНМК — плановые вмешательства откладываются.\n"
                "• Лимит экзогенного эпинефрина у пациентов с кардиопатологией: <b>не более 0.04 мг на весь приём</b> (максимум 2 карпулы 1:100 000 или 4 карпулы 1:200 000).\n"
                "• Категорически избегать внутрисосудистого введения (карпульный шприц с активной аспирацией)."
            )
            await edit_callback_message(bot_client, event, cardio_card, "edit_message:rx_cardio", buttons=rx_back_btns, parse_mode='html')
            await event.answer()
            return

        elif rx_kind == "endo":
            endo_card = (
                "🛡 <b>EBM-Гайдлайн: Антибиотикопрофилактика инфекционного эндокардита (AHA/ESC)</b>\n\n"
                "<b>Показана ТОЛЬКО пациентам группы наивысшего риска:</b>\n"
                "1. Пациенты с любыми протезированными клапанами сердца (включая TAVI) или искусственными материалами пластики клапанов.\n"
                "2. Пациенты с перенесенным инфекционным эндокардитом в анамнезе.\n"
                "3. Пациенты с цианотическими врожденными пороками сердца (нескорригированными или с резидуальными дефектами).\n\n"
                "<i>⚠️ Примечание: Пролапс митрального клапана без тяжелой регургитации и неосложненные пороки профилактики НЕ ТРЕБУЮТ.</i>\n\n"
                "<b>Процедуры с высоким риском бактериемии:</b>\n"
                "• Любые манипуляции с перфорацией слизистой оболочки десны, поддесневой обработкой (SRP), удалением зубов или дентальной имплантацией.\n\n"
            ...
            await edit_callback_message(bot_client, event, endo_card, "edit_message:rx_endo", buttons=rx_back_btns, parse_mode='html')
            await event.answer()
            return

    # 1.3. ОБРАБОТЧИК ДЕМОНСТРАЦИОННОГО КОНСИЛИУМА concilium:*
    if data_str.startswith("concilium:"):
        conc_kind = data_str.split(":", 1)[1]
        conc_back_btns = [
            [Button.inline("🏛 К описанию консилиума", data="nav:concilium")],
            [Button.inline("⬅️ В главное меню", data="nav:main")]
        ]
        if conc_kind == "example":
            example_card = (
                "🏛 <b>Клинический консилиум: Тотальная реабилитация при стираемости и потере зубов</b>\n\n"
                "<b>Пациент:</b> Мужчина 52 года. Генерализованная стираемость твердых тканей зубов II–III степени, снижение высоты нижнего отдела лица (ВНОЛ) на 4 мм. Отсутствуют 1.6, 2.6, 3.6, 4.6. Глубокий резцовый прикус. Хронический апикальный периодонтит 1.1 (PAI 3, коронка разрушена на 70%, феррул сохранен на 1.5 мм палатинально).\n\n"
                "<b>Консенсус коллегии специалистов:</b>\n\n"
                "🔬 <b>Эндодонтист:</b>\n"
                "Зуб 1.1 признан перспективным для сохранения: феррул достаточен для билд-апа на стекловолоконном штифте (СВШ). Протокол: распломбировка, ультразвуковая активация NaOCl 3%, временное вложение пасты Ca(OH)2 на 14 дней, обтурация термопластической гуттаперчей с биокерамическим силером.\n\n"
                "🔪 <b>Хирург-имплантолог:</b>\n"
                "В зонах 1.6, 2.6 показана дентальная имплантация с синус-лифтингом. В зонах 3.6, 4.6 — классическая двухэтапная имплантация. Для долговечности вокруг имплантатов обязательна пластика десны (свободный соединительнотканный трансплантат с нёба) для создания зоны прикрепленной кератинизированной десны шириной не менее 3 мм.\n\n"
                "📐 <b>Ортодонт:</b>\n"
                "Устранение зубоальвеолярного выдвижения антагонистов 1.7 и 2.7 с опорой на ортодонтические мини-винты (интрузия). Нормализация окклюзионной плоскости и выравнивание кривой Шпее.\n\n"
                "👑 <b>Ортопед-гнатолог:</b>\n"
                "Депрограммация жевательных мышц с помощью депрограмматора Койса в течение 3 недель. Определение центрального соотношения челюстей. Повышение межальвеолярной высоты на 4 мм.\n\n"
                "🗺 <b>Согласованный Roadmap лечения:</b>\n"
                "• <b>Фаза 1 (1–2 мес):</b> Санация полости рта, перелечивание 1.1, SRP пародонтальных карманов.\n"
                "• <b>Фаза 2 (3–6 мес):</b> Установка микровинтов (интрузия 1.7, 2.7) + дентальная имплантация 1.6, 2.6, 3.6, 4.6. Временные окклюзионные накладки (overlay) в прикусе.\n"
                "• <b>Фаза 3 (6–9 мес):</b> Тотальное провизорное протезирование на фрезерованных PMMA-коронках (тест-драйв окклюзии и височно-нижнечелюстного сустава 3 месяца).\n"
                "• <b>Фаза 4 (9–10 мес):</b> Постоянная ортопедическая реабилитация (керамические виниры/накладки E.max во фронте, диоксид циркония на имплантатах в боковых сегментах)."
            )
            await edit_callback_message(bot_client, event, example_card, "edit_message:concilium_example", buttons=conc_back_btns, parse_mode='html')
            await event.answer()
            return

    # 1.3. ОБРАБОТЧИКИ ЭКСТРЕННЫХ КЛИНИЧЕСКИХ ПРОТОКОЛОВ sos:*
    if data_str.startswith("sos:"):
        ...
        sos_card_btns = [
            [Button.inline("🔄 Другая ситуация у кресла", data="sos:random")],
            [Button.inline("🚨 К разделам SOS", data="nav:sos"), Button.inline("⬅️ В главное меню", data="nav:main")]
        ]
        await edit_callback_message(bot_client, event, card_data["text"], f"edit_message:sos_{sos_kind}", buttons=sos_card_btns, parse_mode='html')
        await event.answer()
        return

    # 1.4. ОБРАБОТЧИКИ ПЕРЕВОДЧИКА С «ПАЦИЕНТСКОГО» trans:*
    if data_str.startswith("trans:"):
        ...
        trans_card_btns = [
            [Button.inline("🔄 Другой перл пациента", data="trans:random")],
            [Button.inline("🗣 К переводчику", data="nav:translate"), Button.inline("⬅️ В главное меню", data="nav:main")]
        ]
        await edit_callback_message(bot_client, event, card_data["text"], f"edit_message:trans_{trans_kind}", buttons=trans_card_btns, parse_mode='html')
        await event.answer()
        return

    # 1.5. ОБРАБОТЧИКИ БАТЛОВ МАТЕРИАЛОВ vs:*
    if data_str.startswith("vs:"):
        ...
        vs_card_btns = [
            [Button.inline("🔄 Другой батл материалов", data="vs:random")],
            [Button.inline("⚖️ К батлам", data="nav:vs"), Button.inline("⬅️ В главное меню", data="nav:main")]
        ]
        await edit_callback_message(bot_client, event, card_data["text"], f"edit_message:vs_{vs_kind}", buttons=vs_card_btns, parse_mode='html')
        await event.answer()
        return
