...
def _clip_at_sentence(text, limit):
    """
    Обрезает запись по границе предложения, чтобы не оборвать утверждение
    на полуслове: «гипохлорит применяют в концентрации» без числа хуже, чем
