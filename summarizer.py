...
def filter_useful_messages(messages):
    ...
    dental_keywords = {
        ...
        "челюст", "сустав", "окклюз", "прикус",
        # Специализации
        "ортодонт", "хирург", "ортопед", "терапевт", "пародонт", "гнатолог",
        # Диагностика
        "сним", "кт", "рентг", "панорам", "кллт", "оптг", "визиограф",
        # Протоколы и материалы
        "gbt", "srp", "bopt", "ids", "адгези", "цемент", "композит", "керамик", 
        "коффер", "кламп", "матриц", "вертипреп", "преп", "фиксаци", "протокол",
        "клиническ",
        # Оборудование
...
async def process_summary_batch(messages, client, chat_id, topic_id=None, msg_count=0, cached_message=None, delivery_hook=None):
    ...
    **ВЫВОД:** Чему учит этот кейс.
    ...
    Инструкция для самопроверки: Напиши подробный, профессиональный дайджест, но строго следи, чтобы длина текста была в пределах 6000–7500 символов. Это жесткое ограничение, чтобы статья поместилась на Telegraph целиком без обрезки. Пиши максимально плотно, структурировано, излагай только факты, цифры и протоколы, без воды.
    """
...
async def process_weekly_batch(messages, client, chat_id, topic_id=None, delivery_hook=None):
    ...
    *   **Ортодонтия:** (Брекеты, элайнеры...)
    ...
    3. Текст статьи должен быть в пределах 7000–8500 символов, чтобы он полностью поместился на Telegraph без обрезания.
    
    ЛОГ НЕДЕЛИ:
    {full_text}
    Инструкция для самопроверки: Напиши подробную, профессиональную статью, но строго следи, чтобы длина текста была в пределах 7000–8500 символов. Это жесткое ограничение, чтобы статья поместилась на Telegraph целиком без обрезки. Пиши максимально плотно, структурировано, излагай только факты, цифры и протоколы, без воды.
    """
