...
async def process_weekly_batch(messages, client, chat_id, topic_id=None, delivery_hook=None, cached_message=None):
    ...
    # подтянули до 7000–9000, здесь теперь то же число — и оно больше не спорит
    # с «ЖЕСТКИМ ЛИМИТОМ 4000-5000», который стоял ниже в этом же промпте.
    #
    ...
    Твоя задача — написать **ДЕТАЛЬНЫЙ ОБЗОР** по материалам чата стоматологов за неделю в пределах 7000–9000 символов.
    === ПРАВИЛА ВНИМАНИЯ ===
    1. Проанализируй ВЕСЬ предоставленный лог. Не фокусируйся только на последних сообщениях. 
    2. Если в начале или середине лога была важная дискуссия, она ОБЯЗАТЕЛЬНО должна попасть в отчет.
    3. Твоя цель — равномерный охват всех тем за отчетный период.
    ...
    *   **Ортодонтия:** (Брекеты, элайнеры...)
    ...
    4. Объём статьи: 7000-9000 символов. Всё, что выше, будет обрезано вместе с последними разделами.

    ЛОГ НЕДЕЛИ:
    {full_text}
    Инструкция для самопроверки: Напиши максимально подробную, профессиональную, глубокую и развернутую статью в пределах 7000–9000 символов. Это летопись, а не краткое саммари. Излагай клиническую логику, протоколы, цифры и бренды во всех подробностях, концентрируй пользу без воды.
    """
