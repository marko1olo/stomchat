...
if os.path.exists(_savdel):
    _cat = set(re.findall(r'"(\d{1,2}\.\d\.\d)":', io.open(_savdel, encoding="utf-8").read()))
    ...
    check("листья сита вне CAT_MAP объявлены явно, а не молча невидимы",
          (D.LEAF_CODES - _cat) == set(D.NON_EXPORTABLE_LEAVES),
          f"расхождение: {sorted((D.LEAF_CODES - _cat) ^ set(D.NON_EXPORTABLE_LEAVES))}")
else:
...
check("таксономия выведена из дерева, а не переписана пятым списком",
      "re.findall" in CODE.split("LEGAL_CODES")[1][:120],
      "снова ручной список кодов — он разойдётся с деревом")
check("дерево сита совпадает по смыслу с деревом reclass 5.0",
      "2.2.6 Адгезивная и цементная фиксация" in CODE and "3.3.1 Мукогингивальная пластика" in CODE,
